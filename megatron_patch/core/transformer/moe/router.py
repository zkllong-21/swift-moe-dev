...
class TopKRouter(Router):
    ...
    def routing(self, logits: torch.Tensor):
        ...
        # self._log_router_stats(logits, probs)
        self._log_router_stats(logits, probs, routing_map)

        return probs, routing_map

    def _log_router_stats(self, logits: torch.Tensor, probs: torch.Tensor, routing_map: torch.Tensor) -> None:
        """Log router statistics for monitoring."""
        if self.layer_number is None or not self.training or not torch.is_grad_enabled():
            return
        with torch.no_grad():
            logits_f = logits.detach().float()
            logits_abs_mean = logits_f.abs().mean()
            logits_token_var = logits_f.var(dim=0, unbiased=False).mean()

            probs_f = probs.detach().float()
            prob_sum = probs_f.sum(dim=-1, keepdim=True).clamp_min(1e-9)
            p = probs_f / prob_sum
            entropy = -(p * p.clamp_min(1e-9).log()).sum(dim=-1).mean()
            
            # exprt cv
            tokens_per_expert = routing_map.detach().float().sum(dim=0)
            tokens_mean = tokens_per_expert.mean()
            tokens_var = tokens_per_expert.var(unbiased=False)
            tokens_cv = tokens_var / (tokens_mean + 1e-9)
