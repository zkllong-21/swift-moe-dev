...
def save_to_router_stats_tracker(
    ...
    """Save router statistics for logging.

    Args:
        name (str): The name of the statistic.
        value (torch.Tensor): Scalar statistic value.
        layer_number (int): Layer index of the stat.
        num_layers (int): The number of total layers.
        reduce_group (torch.distributed.ProcessGroup): The group for reducing the stat.
        mean_group (torch.distributed.ProcessGroup): The group for averaging the stat.
    """
    if layer_number is None:
        return

    tracker = get_moe_router_stats_tracker()
    if name not in tracker:
        tracker[name] = {}
        tracker[name]["values"] = torch.zeros(num_layers, device=value.device)
    tracker[name]["values"][layer_number - 1] += value.detach()
    tracker[name]["reduce_group"] = reduce_group
    tracker[name]["avg_group"] = avg_group
...
def reduce_router_stats_tracker_across_ranks(track_names: Optional[List[str]] = None):
    """Collect and reduce the router statistics across ranks."""
    tracker = get_moe_router_stats_tracker()
    if track_names is None:
        track_names = tracker.keys()
    for name in track_names:
...
def track_moe_metrics(
    ...
    if track_names:
        aux_values = [
            tracker[name]['values'].float() for name in track_names if name in tracker
        ]
        if aux_values:
            total_aux = torch.stack(aux_values, dim=0).sum(dim=0)
            aux_mean = total_aux.mean()
            aux_var = total_aux.var(unbiased=False)
            aux_cv = aux_var / (aux_mean + 1e-8)
            if total_loss_dict is not None:
                if "moe_aux_loss_cv" not in total_loss_dict:
                    total_loss_dict["moe_aux_loss_cv"] = aux_cv
                else:
                    total_loss_dict["moe_aux_loss_cv"] += aux_cv
            if writer is not None:
                writer.add_scalar("moe_aux_loss_cv", aux_cv, iteration)
            if wandb_writer:
                wandb_writer.log({"moe_aux_loss_cv": aux_cv}, iteration)

    clear_aux_losses_tracker()


def track_moe_router_stats(
    stat_scale: float,
    iteration: int,
    writer,
    wandb_writer=None,
    ...
    """Track MoE router statistics for logging."""
    tracker = get_moe_router_stats_tracker()
    if force_initialize:
        if track_names is not None:
            for key in track_names:
    ...
    reduce_router_stats_tracker_across_ranks(track_names)
    ...
    stat_scale = 1.0 if stat_scale is None else stat_scale
    router_stats = {k: v['values'].float() * stat_scale for k, v in tracker.items()}
    for name, stat_list in router_stats.items():
        if total_loss_dict is not None:
            if name not in total_loss_dict:
                total_loss_dict[name] = stat_list.sum() / num_moe_layers
            else:
                total_loss_dict[name] += stat_list.sum() / num_moe_layers
        if writer is not None:
            writer.add_scalar(name, stat_list.sum() / num_moe_layers, iteration)
            if per_layer_logging:
                for i, stat in enumerate(stat_list.tolist()):
                    writer.add_scalar(f"moe/{name}_layer_{i}", stat, iteration)
        if wandb_writer:
            wandb_writer.log({f"{name}": stat_list.sum() / num_moe_layers}, iteration)
            if per_layer_logging:
                wandb_writer.log(
                    {f"moe/{name}_layer_{i}": stat for i, stat in enumerate(stat_list.tolist())},
                    iteration,
                )
...
def get_moe_router_stats_tracker():
    """Return the moe router stats tracker."""
