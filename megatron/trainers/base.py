...
from megatron.core.transformer.moe.moe_utils import track_moe_metrics, track_moe_router_stats
...
class BaseMegatronTrainer(ABC):
    ...
    def training_log(self, loss_dict, total_loss_dict, learning_rate, decoupled_learning_rate, iteration, loss_scale,
        ...
        if args.num_experts is not None:
            ...
            track_names = []
            load_balancing_type = args.moe_router_load_balancing_type
            if load_balancing_type is None:
                load_balancing_types = []
            elif isinstance(load_balancing_type, str):
                load_balancing_types = [item.strip() for item in load_balancing_type.split(',') if item.strip()]
            elif isinstance(load_balancing_type, (list, tuple, set)):
                load_balancing_types = list(load_balancing_type)
            else:
                load_balancing_types = []
            ...
            track_moe_router_stats(
                stat_scale=moe_loss_scale,
                iteration=iteration,
                writer=writer,
                wandb_writer=wandb_writer,
                total_loss_dict=total_loss_dict,
                per_layer_logging=args.moe_per_layer_logging,
                force_initialize=True,
                track_names=router_track_names,
                num_layers=args.num_layers,
                moe_layer_freq=args.moe_layer_freq,
                **track_moe_kwargs)
